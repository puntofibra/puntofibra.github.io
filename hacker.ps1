...
function R { param([int]$max = 255) Get-Random -Minimum 0 -Maximum $max }
...
Type-Line ("> net.scan(range=`"192.168.{0}.0/24`", stealth=True)" -f (R)) DarkGray 4
Type-Line 'open ports: 22, 443, 8081 - banner grabbing...' Cyan 4
Type-Line ("active host found: 10.{0}.{1}.{2} (TTL=64, linux)" -f (R),(R),(R)) DarkGray 4
...
Type-Line ("trying combination {0}/4096... attempt {1}" -f (R),(R)) DarkGray 4
...
Type-Line ("[WARN] adaptive firewall detected, rerouting via proxy_{0}" -f (R)) Red 4
Type-Line ("spoofing MAC 3C:5A:B4:{0}:{1}:{2} ... done" -f (R),(R),(R)) Cyan 4
...
Type-Line ("[OK] kernel exploit CVE-2024-{0} applied successfully" -f (R)) Cyan 4
